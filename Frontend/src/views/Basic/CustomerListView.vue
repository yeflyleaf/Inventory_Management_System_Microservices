    ...
    <div class="header-section">
      ...
      <div class="filter-group">
        <input 
          ...
          class="search-input"
        />
        <RefreshButton @click="fetchCustomers" />
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="(customer, index) in paginatedCustomers" :key="customer.id">
            ...
            <td class="customer-name" :title="customer.name">{{ customer.name }}</td>
            <td>
              <span class="contact-tag">{{ customer.contactPerson || '-' }}</span>
            </td>
            <td class="phone-cell">
              <div><span class="phone-icon">📞</span> {{ customer.phone || '-' }}</div>
              <div v-if="customer.phone2" class="phone-secondary"><span class="phone-icon">📞</span> {{ customer.phone2 }}</div>
            </td>
            <td class="address-cell" :title="customer.address">{{ customer.address || '-' }}</td>
            <td class="action-cell">
              <button class="btn-small btn-edit" @click="openModal(customer)" title="编辑">
                ✏️ 编辑
              </button>
              <button class="btn-small btn-delete" @click="confirmDelete(customer)" title="删除">
                🗑️ 删除
              </button>
            </td>
          </tr>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-container">
        ...
        <div class="modal-body">
          <form @submit.prevent="saveCustomer" id="customerForm">
            <div class="form-group">
              ...
              <input v-model="form.name" required placeholder="请输入客户名称" class="form-input" />
            </div>
            <div class="form-group">
              <label>联系人</label>
        ...
        <div class="modal-footer">
          ...
          <button type="submit" form="customerForm" class="btn-confirm" :disabled="isSaving">
    ...
    <div v-if="showDeleteModal" class="delete-modal-overlay" @click="closeDeleteModal">
      <div class="delete-modal" @click.stop>
        ...
        <div class="delete-modal-body">
          <p class="delete-warning">您确定要删除以下客户吗？此操作不可撤销！</p>
...
const saveCustomer = async () => {
  ...
  try {
    if (isEdit.value) {
      await axios.put('/customers', form.value)
...
const executeDelete = async () => {
  ...
  try {
    await axios.delete(`/customers/${itemToDelete.value.id}`)
    showSuccess('删除成功')
    // 从列表中移除
    customers.value = customers.value.filter(c => c.id !== itemToDelete.value.id)
    closeDeleteModal()
  } catch (error) {
